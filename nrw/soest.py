...
def soest(sheets):
    ...
    ags, date = 5974, data["Q__timestamp"]
    if not todaystr in date: raise Exception("Soest noch alt: "+str(date))
    ...
    update(sheets, ags, c=c, cc=cc, g=g, gg=gg, d=d, dd=dd, sig="Bot", comment="Bot Dashboard ohne SI", date=date, ignore_delta=True)
    return True
