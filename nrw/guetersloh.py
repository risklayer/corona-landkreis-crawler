...
def guetersloh(sheets):
    ...
    ags, date = 5754, data["datum"]
    if not datetime.date.today().strftime("%d.%m") in date: raise Exception("Essen noch nicht aktuell")
    ...
    update(sheets, ags, c=c, cc=cc, g=g, gg=gg, d=d, dd=dd, sig="Bot", comment="Bot Dashboard ohne SI", date=date)
    return True

schedule.append(Task(11, 25, 13, 10, 300, guetersloh, 5754))
