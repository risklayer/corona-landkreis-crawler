...
def essen(sheets):
    ...
    c, g, d, a, q = None, None, None, None, None
    for f in data["features"]:
        f = f["attributes"]
        if f["STATUS"] == 1: q = f["ANZAKT"]
        if f["STATUS"] == 3: c, a = f["ANZGES"], f["ANZAKT"]
        if f["STATUS"] == 4: g = f["ANZGES"]
        if f["STATUS"] == 5: d = f["ANZGES"]
    date = data["features"][0]["attributes"]["DATEXP"]
    if a and q: q = a + q
    update(sheets, 5113, c=c, g=g, d=d, q=q, sig="Vorläufig", comment="Dashboard ohne SI", date=date, ignore_delta=True)
    return True
