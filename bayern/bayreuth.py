...
_bayreuth_cc = re.compile(r"(\S+) aus dem Landkreis und (\S+) aus der Stadt")
...
def bayreuth(sheets):
    ...
    art = next(x for x in soup.find("section").findAll("article") if "Corona-Fälle in Stadt und Land" in x.get_text())
    ...
    update(sheets, 9462, c=cs, cc=ccs, d=ds, g=gs, sig="Bot", ignore_delta=True) # Stadt
