...
def goerlitz(sheets):
    ...
    assert "Quarant" in rows[5][0]
    a, aa = force_int(rows[5][1].replace("*","")), force_int(rows[5][2].replace("*",""))
    assert "station" in rows[6][0]
    s = force_int(rows[6][1].replace("*",""))
    assert "Tod" in rows[7][0]
    d, dd = force_int(rows[7][1].replace("*","")), force_int(rows[7][2].replace("*",""))
    #print(main.get_text())
