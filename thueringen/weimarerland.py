...
_weimarerland_c = re.compile(r"([0-9.]+) \(([+-][0-9.]+)\) Fälle bisher")
_weimarerland_d = re.compile(r"([0-9.]+) Verstorbene")
_weimarerland_g = re.compile(r"([0-9.]+) genesen")
...
def weimarerland(sheets):
    ...
    d = force_int(_weimarerland_d.search(text).group(1))
    g = force_int(_weimarerland_g.search(text).group(1))
    ...
    q = c - g - d + q1 + q2
    update(sheets, 16071, c=c, cc=cc, d=d, g=g, q=q, s=s, sig="Bot")
    return True
