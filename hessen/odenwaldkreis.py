...
_odenwaldkreis_st = re.compile(r"am \w+ \((\d\d?\. \w+)\)")
_odenwaldkreis_c = re.compile(r"um ([0-9.]+) auf ([0-9.]+)")
...
_odenwaldkreis_g2 = re.compile(r"([0-9.]+|\w+) weitere Personen sind genesen(?: ([0-9.]+|\w))?")
_odenwaldkreis_d1 = re.compile(r"Zahl der Todesfälle bleibt konstant (?:bei )?\(?([0-9.]+)\)?")
_odenwaldkreis_d2 = re.compile(r"([0-9.]+) Todesfälle")
...
def odenwaldkreis(sheets):
    ...
    li = next(x for x in soup.find(id="main").findAll(class_="news-list-item") if "Die Zahl der positiven" in x.get_text() or "Inzidenz" in x.get_text())
    #print(li.get_text())
    ...
    d = force_int((_odenwaldkreis_d1.search(text) or _odenwaldkeis_d2.search(text)).group(1))
