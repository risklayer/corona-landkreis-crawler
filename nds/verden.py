...
_verden_c = re.compile(r"([.0-9]+)\s+\(\+?\s*(-?[.0-9]+)\s+zu\w?\s+(?:\w+|\d+\.\d+\.\d*)\)\s+laborbestätigte\s+Coronavirus-Fälle")
_verden_g = re.compile(r"([.0-9]+)\s+\(\+?\s*(-?[.0-9]+)\s+zu\w?\s+(?:\w+|\d+\.\d+\.\d*)\)\s+Covid-19-Patienten[a-z ]+wieder\s+genesen")
