...
def dithmarschen(sheets):
    ...
    assert "Aktive Fälle gesamt" in rows[3][0]
    ...
    assert "verstorbene" in rows[5][0]
    d, dd = force_int(rows[5][1]), force_int(rows[5][2],0)
    #assert "Quarantäne" in rows[8][0]
    #q = force_int(rows[8][1]) + c - d - g if force_int(rows[8][1]) else None
    g = c - a - d
    update(sheets, 1051, c=c, cc=cc, d=d, dd=dd, g=g, s=s, sig="Bot", ignore_delta=True)
    return True
