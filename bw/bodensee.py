...
def bodensee(sheets):
    ...
    q = data["Quarant_akt"]
    # SI laden
    data = get_json('https://services3.arcgis.com/23E2aUPiEN4SKSFV/arcgis/rest/services/Statistik_Corona_HP_KKHS_neu/FeatureServer/0/query?where=1%3D1&outFields=*&orderByFields=Datum+DESC&resultRecordCount=1&f=json')
    data = data["features"][0]["attributes"]
    #for k,v in data.items(): print(k,v,sep="\t")
